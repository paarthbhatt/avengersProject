...
export function Layout() {
  ...
  const navLinks = [
    ...
    { path: '/thor', name: 'Thor', Icon: Hammer, color: 'hover:text-yellow-400' },
    { path: '/black-widow', name: 'Black Widow', Icon: Heart, color: 'hover:text-red-500' },
    { path: '/hawkeye', name: 'Hawkeye', Icon: Target, color: 'hover:text-purple-400' },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-blue-900/20 to-black">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-b border-blue-500/20">
        <nav className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <Link 
              to="/" 
              className="text-2xl font-bold text-white flex items-center gap-2 hover:scale-105 transition-transform hero-glow"
            >
            ...
            <div className="flex gap-6 overflow-x-auto pb-2">
              {navLinks.map(({ path, name, Icon, color }) => (
                <Link
                  key={path}
                  to={path}
                  className={`text-gray-300 ${color} flex items-center gap-1 relative px-2 py-1 transition-colors ${
      ...
      <main className="container mx-auto px-4 py-24">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="hero-gradient p-8 rounded-lg shadow-2xl backdrop-blur-sm"
          >
            <Outlet />
          </motion.div>
        </AnimatePresence>
      ...
      {/* Footer */}
      <footer className="bg-black/80 backdrop-blur-md border-t border-blue-500/20 py-12 mt-12">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              <h3 className="text-xl font-bold text-white mb-4 hero-glow">The Avengers</h3>
              <p className="text-gray-400">
                Earth's Mightiest Heroes stand as the planet's first line of defense against the world's most powerful threats.
              </p>
            </div>
            <div>
              <h3 className="text-xl font-bold text-white mb-4 hero-glow">Quick Links</h3>
            ...
            <div>
              <h3 className="text-xl font-bold text-white mb-4 hero-glow">Connect</h3>
